...
<div class="container mt-5 mb-5">
        ...
        <div class="d-flex justify-content-center row" >
            <div class="d-flex justify-content-end col-md-8 mr-2 mb-1">
                <a class="fas fa-arrow-left fa-sm" href="{{ route('front.showsingelpost', $post->id)}}"> back</a>
            </div>
        ...
        @endif
            ...
            <div class="d-flex flex-column col-md-8">
                <div class=" align-items-center text-left comment-top p-2 bg-white border-bottom px-4">
                    ...
                    @if($post->user_id == auth()->user()->id)
                    <form method="POST" action="{{ route('front.editPCR',['id' => $post->id,'type'=>'Post']) }}">
                        @csrf
                        <div class="comment-text-sm mt-2" style="margin-left:10px ">
                            <textarea style="font-weight: bold; width: 100%;" name="title" rows="3">{{ old('title', $post->title ?? '') }}</textarea>
                        </div>

                        <div class="comment-text-sm" style="margin-left:10px">
                            <textarea style="width: 100%;" name="body" rows="3">{{ old('body', $post->body ?? '') }}</textarea>
                                
                        </div>
                        <div class="d-flex justify-content-end mb-3">
                            <button class="btn btn-success btn-sm" style="font-size: 10px" type="submit">Submit</button>
                        </div>
                    </form>
                ...
                <div class="coment-bottom bg-white p-2 px-4 mb-5 pb-4">
                    ...
                    <div class="commented-section my-2 box-shadow py-2">
                           ...
                           @if($comment->user_id == auth()->user()->id)
                                        <div class="action-texts">
                                            <form action="{{ route('front.deletePCR', ['id' => $comment->id,'type'=>'Comment']) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('POST')
                                                <button type="submit" class="remove-text mr-3" title="Remove" >remove</button>
                                            </form>
                                        </div>
                             @endif
                       ...
                       @if($comment->user_id == auth()->user()->id)
                            ...
                            <form method="post" action="{{ route('front.editPCR',['id' => $comment->id,'type'=>'Comment']) }}">
                                @csrf
                                <div class="comment-text-sm " style="margin-left:90px">
                                    <textarea style="width: 90%;" name="comment" rows="3" cols="30">{{ old('comment', $comment->comment ?? '') }}</textarea>
                                
                                </div>
                                <div class="d-flex justify-content-end mb-3">
                                    <button class="btn btn-success btn-sm mr-3" style="font-size: 10px" type="submit">Submit</button>
                                </div>
                            </form>
                        @else
